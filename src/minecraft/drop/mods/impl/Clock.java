...
public class Clock extends ModDraggable {    
	...
	@Override
	public int getWidth() {
		return options.getBooleanOption("showBackground").isToggled() ? 53 : font.getStringWidth(Brackets.fromId((int) options.getStepOption("brackets").getValue()).wrap("12:00"));
	}
	...
	public void render(ScreenPosition pos) {
		if (options.getBooleanOption("showBackground").isToggled()) {
		    ...
		    drawAlignedText(Brackets.fromId((int) options.getStepOption("brackets").getValue()).wrap(LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm"))), pos.getAbsoluteX() + 1, pos.getAbsoluteY() + 1, options.getColorOption("textColor").getColor(), options.getBooleanOption("textShadow").isToggled());
    	}
	}
	...
	public void renderDummy(ScreenPosition pos) {
		if (options.getBooleanOption("showBackground").isToggled()) {
		    ...
		    drawAlignedText(Brackets.fromId((int) options.getStepOption("brackets").getValue()).wrap("12:00"), pos.getAbsoluteX() + 1, pos.getAbsoluteY() + 1, options.getColorOption("textColor").getColor(), options.getBooleanOption("textShadow").isToggled());
    	}
	}
}
