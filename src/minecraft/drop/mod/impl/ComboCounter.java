...
public class ComboCounter extends ModDraggable {
	...
	@Override
	public int getWidth() {
		return options.getBooleanOption("showBackground").isToggled() ? 53 : font.getStringWidth(Brackets.fromId((int) options.getEnumOption("brackets").getValue()).wrap("0 combo"));
	}
	...
	public void render(ScreenPosition pos) {
		...
		if (options.getBooleanOption("showBackground").isToggled()) {
		    ...
		    drawAlignedText(Brackets.fromId((int) options.getEnumOption("brackets").getValue()).wrap(combo + " combo"), pos.getAbsoluteX() + 1, pos.getAbsoluteY() + 1, options.getColorOption("textColor").getColor(), options.getBooleanOption("textShadow").isToggled());
    	}
	}
	...
	public void renderDummy(ScreenPosition pos) {
		if (options.getBooleanOption("showBackground").isToggled()) {
		    ...
		    drawAlignedText(Brackets.fromId((int) options.getEnumOption("brackets").getValue()).wrap("0 combo"), pos.getAbsoluteX() + 1, pos.getAbsoluteY() + 1, options.getColorOption("textColor").getColor(), options.getBooleanOption("textShadow").isToggled());
    	}
	}
