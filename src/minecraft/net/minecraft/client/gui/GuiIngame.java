...
public class GuiIngame extends Gui
{
    ...
    private void renderBossHealth()
    {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0)
        {
            ...
            int i1 = 12;
            int i2 = ModInstances.getBossbarMod().isShowNameToggled() ? 0 : mc.fontRendererObj.FONT_HEIGHT;
            
            if (ModInstances.getBossbarMod().isShowHealthToggled()) {
            	this.drawTexturedModalRect(k, i1 - i2, 0, 74, j, 5);
                this.drawTexturedModalRect(k, i1 - i2, 0, 74, j, 5);

                if (l > 0)
                {
                    this.drawTexturedModalRect(k, i1 - i2, 0, 79, l, 5);
                }
            }

            if (ModInstances.getBossbarMod().isShowNameToggled()) {
