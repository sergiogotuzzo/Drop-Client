package rubik.gui;

import java.io.IOException;

import org.apache.commons.lang3.StringUtils;

import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.GuiButton;
import net.minecraft.client.gui.GuiListExtended;
import rubik.events.EventTarget;
import rubik.events.impl.ClickEvent;
import rubik.mods.Mod;

public class ModEntry implements GuiListExtended.IGuiListEntry, Comparable<ModEntry> {
	private final Mod mod;
	private final GuiModsListNew inGui;
	private final GuiCheckBox checkbox;
	private final GuiSettingsButton settings;
	private final String name;
	
	public ModEntry(GuiModsListNew inGui, Mod mod, GuiRubikClientScreen settingsGui) {
		this.mod = mod;
		this.inGui = inGui;
		
		name = StringUtils.join(StringUtils.splitByCharacterTypeCamelCase(mod.getClass().getSimpleName().replace("Mod", "").replaceAll("\\d+", "")), " ");
		checkbox = new GuiCheckBox(0, 0, 0, mod.isEnabled());
		settings = new GuiSettingsButton(0, 0, 0, 20, 20, settingsGui);
	}
	
	@Override
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
		this.checkbox.xPosition = x + 200 - 20;
		this.checkbox.yPosition = y;
		
		this.settings.xPosition = x + 200;
		this.settings.yPosition = y;
		
		this.checkbox.drawButton(Minecraft.getMinecraft(), mouseX, mouseY);
		this.settings.drawButton(Minecraft.getMinecraft(), mouseX, mouseY);
		
		this.mod.setEnabled(this.checkbox.isChecked());
		
		inGui.drawString(Minecraft.getMinecraft().fontRendererObj, name, x, y + 4, -1);
	}

	@Override
	public boolean mousePressed(int slotIndex, int x, int y, int p_148278_4_, int p_148278_5_, int p_148278_6_) {
		if (this.checkbox.mousePressed(Minecraft.getMinecraft(), x, y)) {
			return true;
		} else if (this.settings.mousePressed(Minecraft.getMinecraft(), x, y)) {
			return true;
		}

		return false;
	}

	@Override
	public void mouseReleased(int slotIndex, int x, int y, int mouseEvent, int relativeX, int relativeY) {
		this.checkbox.mouseReleased(x, y);
		this.settings.mouseReleased(x, y);
	}
	
	@Override
	public void setSelected(int p_178011_1_, int p_178011_2_, int p_178011_3_) {
		
	}

	@Override
	public int compareTo(ModEntry o) {
		return this.name.compareTo(o.name);
	}
}